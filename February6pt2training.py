# -*- coding: utf-8 -*-
"""
Created on Tue Feb  6 23:28:35 2024
...
class VGGPerceptualLoss(nn.Module):
    def __init__(self, resize=True):
        ...
        # Adjusting the instantiation of VGG models to use the weights parameter
...
class InpaintingDataset(Dataset):
    ...
    def create_weight_map(self, mask, dilated_mask, border_weight=2.0):
        # Assuming mask and dilated_mask are binary [0, 1] tensors
        border = dilated_mask - mask
        weight_map = torch.ones_like(mask)
        # Apply a graduated weight based on the distance from the original mask edge
        # For simplicity, this example sets a constant weight for the border,
        # but you could use a gradient or a distance transform for more nuanced weighting
        weight_map += border * (border_weight - 1)
        return weight_map
...
# Initialize learning rate schedulers outside the training loop
...
perceptual_loss_fn.mean = perceptual_loss_fn.mean.to(device)
perceptual_loss_fn.std = perceptual_loss_fn.std.to(device)
...
def validate_model(generator, val_dataloader, device):
    generator.eval()
    total_l1_loss = 0.0
    total_l2_loss = 0.0
    total_psnr = 0.0
    ...
    total_loss=total_l1_loss+total_l2_loss+total_psnr + total_ssim + total_inception_score
...
scaler = GradScaler()
with open('validation_metrics3.txt', 'w') as validation_metrics_file:
    for epoch in range(num_epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            with autocast():
                ...
                g_loss = gan_loss_fake + pixel_loss + edge_loss + perceptual_loss
            ...
            # scaler.unscale_(optimizer_G)  # Unscale the gradients of optimizer_G before clipping
            # scaler.unscale_(optimizer_D)
            
            # Right after your backward() calls and before optimizer.step() calls for both G and D:
            ...
            if i % print_interval == 0:            
                print(f"[Epoch {epoch}/{num_epochs}] [Batch {i}/{len(train_dataloader)}] "
                      ...
                      f"[GAN loss (global): {gan_loss_fake.item()}] "
            ...
            if i% validation_interval and i>0:
                validation_loss = validate_model(generator, val_dataloader, device)
                # Log validation loss
                validation_metrics_file.write(f'Epoch {epoch}: Validation Loss: {validation_loss}\n')
